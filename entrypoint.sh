...
exchange_correspond=${FETCH_EX_COR}
exchange_comment=${FETCH_EX_COM}
...
if [ "${array_type[$i]}" = "comment" ]; then
	echo "username ${array_user[$i]} password ${array_pass[$i]}  mda \"/usr/bin/amqp-publish -e '${exchange_comment}' -r '${array_queue[$i]}' --url='${amqp_connect}'\" options ssl" >> ${fetch_conf}
else
	echo "username ${array_user[$i]} password ${array_pass[$i]}  mda \"/usr/bin/amqp-publish -e '${exchange_correspond}' -r '${array_queue[$i]}' --url='${amqp_connect}'\" options ssl" >> ${fetch_conf}
fi
